...
public class FileBasedConfiguration extends FtpConfiguration {
	...
	private static final XmlDecl[] configIdentityDecls = {
			// identity
			new XmlDecl(XmlType.STRING, XML_SERVER_HOSTID), new XmlDecl(XmlType.STRING, XML_PATH_CRYPTOKEY),
	...
	private static final XmlDecl[] configServerParamDecls = {
			// server
			new XmlDecl(XmlType.BOOLEAN, XML_USELOCALEXEC), new XmlDecl(XmlType.STRING, XML_LEXECADDR),
			new XmlDecl(XmlType.INTEGER, XML_LEXECPORT), new XmlDecl(XmlType.STRING, XML_SERVER_ADMIN),
			new XmlDecl(XmlType.STRING, XML_SERVER_PASSWD), new XmlDecl(XmlType.BOOLEAN, XML_USEHTTPCOMP),
			...
			new XmlDecl(XmlType.STRING, XML_PATH_ADMIN_KEYPASS), new XmlDecl(XmlType.STRING, XML_MONITOR_SNMP_CONFIG) };
	/**
	 * SERVER PORT
	 */
	...
	private static final XmlDecl[] configNetworkServerDecls = {
			// network
			new XmlDecl(XmlType.INTEGER, XML_SERVER_PORT), new XmlDecl(XmlType.STRING, XML_SERVER_ADDRESS),
			new XmlDecl(XmlType.INTEGER, XML_RANGE_PORT_MIN), new XmlDecl(XmlType.INTEGER, XML_RANGE_PORT_MAX),
			new XmlDecl(XmlType.INTEGER, XML_SERVER_HTTP_PORT), new XmlDecl(XmlType.INTEGER, XML_SERVER_HTTPS_PORT) };
	/**
	...
	private static final XmlDecl[] configDbDecls = {
			// db
			new XmlDecl(XmlType.STRING, XML_DBDRIVER), new XmlDecl(XmlType.STRING, XML_DBSERVER),
			new XmlDecl(XmlType.STRING, XML_DBUSER), new XmlDecl(XmlType.STRING, XML_DBPASSWD) };
	/**
	...
	private static final XmlDecl[] configLimitDecls = {
			// limit
			new XmlDecl(XmlType.BOOLEAN, XML_DELETEONABORT), new XmlDecl(XmlType.LONG, XML_LIMITSESSION),
			new XmlDecl(XmlType.LONG, XML_LIMITGLOBAL), new XmlDecl(XmlType.LONG, XML_LIMITDELAY),
			new XmlDecl(XmlType.INTEGER, XML_SERVER_THREAD), new XmlDecl(XmlType.INTEGER, XML_CLIENT_THREAD),
			new XmlDecl(XmlType.LONG, XML_MEMORY_LIMIT), new XmlDecl(XmlType.BOOLEAN, XML_CSTRT_USECPULIMIT),
			new XmlDecl(XmlType.BOOLEAN, XML_CSTRT_USECPUJDKLIMIT), new XmlDecl(XmlType.DOUBLE, XML_CSTRT_CPULIMIT),
			new XmlDecl(XmlType.INTEGER, XML_CSTRT_CONNLIMIT), new XmlDecl(XmlType.DOUBLE, XML_CSTRT_LOWCPULIMIT),
			new XmlDecl(XmlType.DOUBLE, XML_CSTRT_HIGHCPULIMIT), new XmlDecl(XmlType.DOUBLE, XML_CSTRT_PERCENTDECREASE),
			new XmlDecl(XmlType.LONG, XML_CSTRT_LIMITLOWBANDWIDTH), new XmlDecl(XmlType.LONG, XML_CSTRT_DELAYTHROTTLE),
			new XmlDecl(XmlType.LONG, XML_TIMEOUTCON), new XmlDecl(XmlType.BOOLEAN, XML_USENIO),
			new XmlDecl(XmlType.BOOLEAN, XML_USEFASTMD5), new XmlDecl(XmlType.STRING, XML_FASTMD5),
	...
	private static final XmlDecl[] configExecDecls = {
			// Exec
			new XmlDecl(XmlType.STRING, XML_RETRIEVE_COMMAND), new XmlDecl(XmlType.LONG, XML_DELAYRETRIEVE_COMMAND),
			new XmlDecl(XmlType.STRING, XML_STORE_COMMAND), new XmlDecl(XmlType.LONG, XML_DELAYSTORE_COMMAND) };
	/**
	 * Base Directory
	 */
	...
	private static final XmlDecl[] configSslDecls = {
			// ssl
			new XmlDecl(XmlType.STRING, XML_PATH_KEYPATH), new XmlDecl(XmlType.STRING, XML_PATH_KEYSTOREPASS),
			new XmlDecl(XmlType.STRING, XML_PATH_KEYPASS), new XmlDecl(XmlType.STRING, XML_PATH_TRUSTKEYPATH),
			new XmlDecl(XmlType.STRING, XML_PATH_TRUSTKEYSTOREPASS),
			new XmlDecl(XmlType.BOOLEAN, XML_USECLIENT_AUTHENT), new XmlDecl(XmlType.BOOLEAN, XML_IMPLICIT_FTPS),
	 ...
	 * Authentication Fields
	 */
	private static final String XML_AUTHENTIFICATION_BASED = "/" + XML_AUTHENTIFICATION_ROOT + "/"
	 ...
	 * Global Structure for Server Configuration
	 */
	private static final XmlDecl[] authentElements = { new XmlDecl(XML_AUTHENTIFICATION_ENTRY, XmlType.XVAL,
	 ...
	 */
	public FileBasedConfiguration(Class<?> classtype, Class<? extends BusinessHandler> businessHandler,
			Class<? extends DataBusinessHandler> dataBusinessHandler, FileParameterInterface fileParameter) {
	...
	private boolean loadAuthentication() {
		...
		if (value != null && (!value.isEmpty())) {
			//TODO
			File configFile = new File(FileBasedConfiguration.class.getClassLoader().getResource(value.getString()).getFile());
			setAuthenticationFile(configFile.toString());
	...
	private boolean loadServerParam() {
		...
		try {
			...
			logger.error("Unable to Decrypt Server Password in Config file from: " + passwd, e);
			return false;
		}
		...
		if (path == null || path.length() == 0) {
			...
			File file = new File(FileBasedConfiguration.class.getClassLoader().getResource(path).getFile());
			if (!file.isDirectory()) {
				logger.error("Http Admin is not a directory in Config file");
				return false;
			}
			try {
				httpBasePath = FilesystemBasedDirImpl.normalizePath(file.getCanonicalPath()) + DirInterface.SEPARATOR;
		...
		if (httpBasePath != null) {
			...
			if (value != null && (!value.isEmpty())) {
				File configFile = new File(
						FileBasedConfiguration.class.getClassLoader().getResource(value.getString()).getFile());
				String keypath = configFile.toString();
				...
				try {
					HttpSslInitializer.waarpSecureKeyStore = new WaarpSecureKeyStore(keypath, keystorepass, keypass);
				...
				HttpSslInitializer.waarpSslContextFactory = new WaarpSslContextFactory(
	...
	private boolean loadLimit(boolean updateLimit) {
		...
		if (highcpuLimit > 0) {
			constraintLimitHandler = new FtpConstraintLimitHandler(getTIMEOUTCON(), useCpuLimit, useCpuLimitJDK,
					cpulimit, connlimit, lowcpuLimit, highcpuLimit, percentageDecrease, null, delay, limitLowBandwidth);
		} else {
			constraintLimitHandler = new FtpConstraintLimitHandler(getTIMEOUTCON(), useCpuLimit, useCpuLimitJDK,
					cpulimit, connlimit);
		}
	...
	private boolean loadDatabase() {
		...
		if (value == null || (value.isEmpty())) {
			...
			try {
				DbConstant.gatewayAdmin = DbModelFactory.initialize(dbdriver, dbserver, dbuser, dbpasswd, true);
	...
	protected boolean loadSsl() {
		...
		if (value == null || (value.isEmpty())) {
			...
			try {
				FtpsInitializer.waarpSecureKeyStore = new WaarpSecureKeyStore(keypath, keystorepass, keypass);
			} catch (CryptoException e) {
		...
		if (value == null || (value.isEmpty())) {
			...
			try {
				FtpsInitializer.waarpSecureKeyStore.initTrustStore(keypath, keystorepass, useClientAuthent);
			} catch (CryptoException e) {
				logger.error("Bad TrustKeyStore construction");
				return false;
			}
		}
		FtpsInitializer.waarpSslContextFactory = new WaarpSslContextFactory(FtpsInitializer.waarpSecureKeyStore);
	...
	public void configureHttps() {
		...
		httpExecutor = new NioEventLoopGroup(getSERVER_THREAD() * 10, new WaarpThreadFactory("HttpExecutor"));
		...
		logger.warn("Start Https Support on port: " + SERVER_HTTPSPORT + " with " + (useHttpCompression ? "" : "no")
				+ " compression support");
	...
	public void configureConstraint() {
		constraintLimitHandler.setHandler(this.getFtpInternalConfiguration().getGlobalTrafficShapingHandler());
	}
	 ...
	 * @param purge
	 *            if True, the current authentications are totally replaced by
	 *            the new ones
	 * @return True if OK
	 */
	...
	public boolean initializeAuthent(String filename, boolean purge) {
		...
		try {
			...
			logger.error("Unable to read the XML Authentication file: " + filename, e);
			return false;
		}
		if (document == null) {
		...
		ConcurrentHashMap<String, SimpleAuth> newAuthents = new ConcurrentHashMap<String, SimpleAuth>();
		for (XmlValue[] xmlValues : list) {
			...
			if (value != null && (!value.isEmpty())) {
				...
				File key = new File(FileBasedConfiguration.class.getClassLoader().getResource(value.getString()).getFile());
			...
			SimpleAuth auth = new SimpleAuth(user, passwd, account, storcmd, stordelay, retrcmd, retrdelay);
	...
	public String getHtmlAuth(String format) {
		...
		/*
		 * XXXUSERXXX XXXPWDXXX XXXACTSXXX XXXADMXXX XXXSTCXXX XXXSTDXXX
		 * XXXRTCXXX XXXRTDXXX
		 */
	...
	public String getHtmlTransfer(String format, int limit) {
		...
		StringBuilder builder = new StringBuilder();
		/*
		 * XXXIDXXX XXXUSERXXX XXXACCTXXX XXXFILEXXX XXXMODEXXX XXXSTATUSXXX
		 * XXXINFOXXX XXXUPINFXXX XXXSTARTXXX XXXSTOPXXX
		 */
		...
		try {
			try {
				preparedStatement = DbTransferLog.getStatusPrepareStament(DbConstant.gatewayAdmin.getSession(), null,
						limit);
	 ...
	 * @author Frederic Bregier
	 */
	private static class GgChannelGroupFutureListener implements ChannelGroupFutureListener {
		EventExecutorGroup executorBoss;
		EventExecutorGroup executorWorker;
		String name;

		public GgChannelGroupFutureListener(String name, EventExecutorGroup executorBoss,
				EventExecutorGroup executorWorker) {
			this.name = name;
	...
	public void releaseResources() {
		...
		if (httpChannelGroup != null) {
			...
			httpChannelGroup.close()
					.addListener(new GgChannelGroupFutureListener("HttpChannelGroup", bossGroup, workerGroup));
		}
	...
	public void inShutdownProcess() {
		if (ftpMib != null) {
			ftpMib.notifyStartStop("Shutdown in progress for " + HOST_ID, "Gives extra seconds: " + getTIMEOUTCON());
		}
	}
