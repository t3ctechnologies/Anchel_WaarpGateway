...
public class DbModelMysql extends org.waarp.common.database.model.DbModelMysql {
	...
	public void createTables(DbSession session) throws WaarpDatabaseNoConnectionException {
		...
		for (int i = 0; i < acolumns.length; i++) {
			action += acolumns[i].name() + DBType.getType(DbTransferLog.dbTypes[i]) + notNull + ", ";
		}
		...
		// cptrunner
		/*
		 * # Table to handle any number of sequences: CREATE TABLE Sequences (
		 * name VARCHAR(22) NOT NULL, seq INT UNSIGNED NOT NULL, # (or BIGINT)
		 * PRIMARY KEY name ); # Create a Sequence: INSERT INTO Sequences (name,
		 * seq) VALUES (?, 0); # Drop a Sequence: DELETE FROM Sequences WHERE
		 * name = ?; # Get a sequence number: UPDATE Sequences SET seq =
		 * LAST_INSERT_ID(seq + 1) WHERE name = ?; $seq = $db->LastInsertId();
		 */
		action = "CREATE TABLE Sequences (name VARCHAR(22) NOT NULL PRIMARY KEY," + "seq BIGINT NOT NULL)";
		System.out.println(action);
		try {
			if (!action.contains("Sequences")) {
				request.query(action);
			}
		} catch (WaarpDatabaseNoConnectionException e) {
			e.printStackTrace();
	...
	public void resetSequence(DbSession session, long newvalue) throws WaarpDatabaseNoConnectionException {
		String action = "UPDATE Sequences SET seq = " + newvalue + " WHERE name = '" + DbTransferLog.fieldseq + "'";
		DbRequest request = new DbRequest(session);
		try {
			request.query(action);
		} catch (WaarpDatabaseNoConnectionException e) {
	...
	public synchronized long nextSequence(DbSession dbSession)
			throws WaarpDatabaseNoConnectionException, WaarpDatabaseSqlException, WaarpDatabaseNoDataException {
		lock.lock();
		try {
			long result = DbConstant.ILLEGALVALUE;
			String action = "SELECT seq FROM Sequences WHERE name = '" + DbTransferLog.fieldseq + "' FOR UPDATE";
			...
			action = "UPDATE Sequences SET seq = " + (result + 1) + " WHERE name = '" + DbTransferLog.fieldseq + "'";
			try {
				preparedStatement.createPrepareStatement(action);
				// Limit the search
